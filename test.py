...
checkpoint_dir = './checkpointtest/end_to_end/'
ckpt_path = os.path.join(checkpoint_dir, 'model_epoch_2400.ckpt')

edge_model = SwinIR(
    upscale=1, img_size=(64, 64), window_size=8, img_range=1., depths=[1],
...
test_dataset = LLVIP_data('./dataset/test/test_52')
test_loader = DataLoader(
    test_dataset, batch_size=1, shuffle=False,
    num_workers=0, pin_memory=True
)

eval_vi_data_names = glob('./dataset/test/test_52/visible/*.jpg')
...
with torch.no_grad():
    for idx, (Inf, vi, name) in enumerate(test_tqdm):
        ...
        I_fusion = fusion_model(Inf, fea_vi, edge, edge)
        ...
        del vi_eval, vi_eval_tensor, vi_3_eval, vi_3_eval_tensor, I_fusion, I_fusion_rgb
        if device.type == 'cuda':
