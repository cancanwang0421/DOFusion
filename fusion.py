...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=224, patch_size=4, resi_connection=None):
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
                 ...
                 use_checkpoint=False, upscale=2, img_range=1., upsampler='', resi_connection=None,
                 **kwargs):
        ...
        if resi_connection == '1conv':
            ...
            self.conv_after_body = nn.Conv2d(embed_dim, embed_dim, 1, 1, 0)  # 使用1x1卷积
...
class Attention(nn.Module):
    def __init__(self, in_channels=8, num_head=1, ratio=1):  # 通道数调整为8
...
class encoder_ir(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 8, 3, 1, 1)  # 直接输出8通道
...
class decoder_fusion(nn.Module):
    def __init__(self):
        ...
        self.con2 = nn.Conv2d(8, 4, 3, 1, 1)
        self.con3 = nn.Conv2d(4, 1, 3, 1, 1)
        self.lrelu = torch.nn.LeakyReLU()
    def forward(self, x):
        x = self.con2(x)
        x = self.lrelu(x)
...
class fusion(nn.Module):
    ...
    def forward(self, x_ir, fea_vi, edge_shallow, edge_deep):
        fea_ir = self.ir_encoder(x_ir)
        
        fea_vi = self.adjust_vi(fea_vi) 
        
        fea_cat = torch.cat([fea_vi, fea_ir], dim=1)
        fea_conv1 = self.lrelu(self.conv1(fea_cat))
        fea_conv2 = self.lrelu(self.conv2(fea_conv1))
        fea_cat_12 = torch.cat([fea_conv1, fea_conv2], dim=1)
        fea_conv3 = self.lrelu(self.conv3(fea_cat_12))
        fea_cat_123 = torch.cat([fea_conv1, fea_conv2, fea_conv3], dim=1) 
        fea_conv4 = self.lrelu(self.conv4(fea_cat_123))
        fea_conv5 = self.lrelu(self.conv5(fea_conv4))
        
        edge_s_1 = self.shallow(edge_shallow)  # [B, 8, H, W]
        fea_fusion = fea_conv5 * edge_s_1
        
        edge_deep = self.deep(edge_deep)
        fea_fusion = self.attention(fea_fusion, edge_deep)
